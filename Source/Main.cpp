...
void Cheat::GameFunctions::GUILoop()
{
	if (GetAsyncKeyState(VK_MULTIPLY) & 0x1 && Cheat::CheatFunctions::IsGameWindowFocussed() || GetAsyncKeyState(VK_F4) & 0x1 && Cheat::CheatFunctions::IsGameWindowFocussed()) 
	{
		if (Cheat::Controls::isMenuEnabled) { Cheat::Controls::isMenuEnabled = false; } else { Cheat::Controls::isMenuEnabled = true; } 
	}

	Cheat::Controls::CheckKeys();

	if (Cheat::Controls::isMenuEnabled) {
		if (Cheat::Controls::currentMenu("mainmenu")) {
			...
			GUI::MenuOption("Settings >", "MenuSettingsMenu");
		}
		if (Cheat::Controls::currentMenu("CommonTeleportsMenu"))
		{
		...
		if (Cheat::Controls::currentMenu("TeleportOptionsMenu"))
		{
		...
		if (Cheat::Controls::currentMenu("CustomTeleportLocationsMenu"))
		{
		...
		if (Cheat::Controls::currentMenu("TimeSettingsMenu"))
		{
		...
		if (Cheat::Controls::currentMenu("WeatherOptionsMenu"))
		{
		...
		if (Cheat::Controls::currentMenu("WorldOptionsMenu"))
		{
		...
		if (Cheat::Controls::currentMenu("PedSpawnerMenu"))
		{
			...
			{
				MISC::DISPLAY_ONSCREEN_KEYBOARD(true, "FMMC_KEY_TIP", "", "", "", "", "", 50);
				while (MISC::UPDATE_ONSCREEN_KEYBOARD() == 0) WAIT(0);
		...
		if (Cheat::Controls::currentMenu("MenuSettingsMenu"))
		{
			GUI::Title("Settings");
		...
		if (Cheat::Controls::currentMenu("MiscOptionsMenu"))
		{
		...
		if (Cheat::Controls::currentMenu("GUISettingsMenu"))
		{
			GUI::Title("GUI Settings");
			GUI::FloatOption("Menu X-Position", &Cheat::GUI::MenuXFloat, 180.f, 1100.f, 10, "Change MenuX Position");
		}
		if (Cheat::Controls::currentMenu("HudOptionsMenu"))
		{
			GUI::Title("HUD Options");
			GUI::BoolOption("Hide HUD", &Cheat::CheatFeatures::HideHUDBool, "Hide all HUD elements");
		}
		if (Cheat::Controls::currentMenu("SelfMenu")) {
			...
			if (GUI::Option("Clean Player", "Remove any damage from your character")) {
				PED::CLEAR_PED_BLOOD_DAMAGE(PLAYER::PLAYER_PED_ID());
			}
			if (GUI::Option("Suicide", "Kill your player")) {
				ENTITY::_SET_ENTITY_HEALTH(PLAYER::PLAYER_PED_ID(), 0, 0);
			}
		}
		if (Cheat::Controls::currentMenu("ModelChangerMenu"))
		{
		...
		if (Cheat::Controls::currentMenu("PlayerlistMenu"))
		{
		...
		if (Cheat::Controls::currentMenu("NetworkingOptionsMenu"))
		{
			GUI::Title("Network Options");
			GUI::MenuOption("Player List >", "PlayerlistMenu");
		}
		if (Cheat::Controls::currentMenu("SelectedPlayerMenu"))
		{
			if (!ENTITY::DOES_ENTITY_EXIST(PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(Cheat::CheatFeatures::OnlineSelectedPlayer))) { Cheat::Controls::changeMenu("PlayerlistMenu"); Cheat::GameFunctions::PrintSubtitle("~COLOR_RED~Invalid Player"); }
		...
		if (Cheat::Controls::currentMenu("SelectedPlayerSpawnOptionsMenu"))
		{
			if (!ENTITY::DOES_ENTITY_EXIST(PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(Cheat::CheatFeatures::OnlineSelectedPlayer))) { Cheat::Controls::changeMenu("PlayerlistMenu"); Cheat::GameFunctions::PrintSubtitle("~COLOR_RED~Invalid Player"); }
			GUI::Title("Spawn Options");
			GUI::MenuOption("Ped Spawner >", "SelectedPlayerPedSpawnerMenu");
		}
		if (Cheat::Controls::currentMenu("SelectedPlayerPedSpawnerMenu"))
		{
			if (!ENTITY::DOES_ENTITY_EXIST(PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(Cheat::CheatFeatures::OnlineSelectedPlayer))) { Cheat::Controls::changeMenu("PlayerlistMenu"); Cheat::GameFunctions::PrintSubtitle("~COLOR_RED~Invalid Player"); }
			GUI::Title("Ped Spawner");
			GUI::BoolOption("Spawn Ped Dead", &Cheat::CheatFeatures::SpawnPedDeadBool, "Usefull for selling");
			if (GUI::Option("Custom Input", "")) { MISC::DISPLAY_ONSCREEN_KEYBOARD(true, "FMMC_KEY_TIP", "", "", "", "", "", 50); while (MISC::UPDATE_ONSCREEN_KEYBOARD() == 0) WAIT(0); const char* SpawnPed = MISC::GET_ONSCREEN_KEYBOARD_RESULT(); if (MISC::GET_ONSCREEN_KEYBOARD_RESULT()) { Cheat::GameFunctions::SpawnPed(SpawnPed, PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(Cheat::CheatFeatures::OnlineSelectedPlayer)); } }
		...
		if (Cheat::Controls::currentMenu("SelectedPlayerTrollMenu"))
		{
			if (!ENTITY::DOES_ENTITY_EXIST(PLAYER::GET_PLAYER_PED_SCRIPT_INDEX(Cheat::CheatFeatures::OnlineSelectedPlayer))) { Cheat::Controls::changeMenu("PlayerlistMenu"); Cheat::GameFunctions::PrintSubtitle("~COLOR_RED~Invalid Player"); }
		...
		if (Cheat::Controls::currentMenu("VehicleOptionsMenu"))
		{
		...
		if (Cheat::Controls::currentMenu("VehicleSpawnSettingsMenu"))
		{
		...
		if (Cheat::Controls::currentMenu("VehicleSpawnerMenu"))
		{
		...
		if (Cheat::Controls::currentMenu("WeaponOptionsMenu")) {
		...
		if (Cheat::Controls::currentMenu("TriggerBotMenu")) {
...
{
	Cheat::CheatFunctions::CheatInitialization();
	return 0;
}
...
BOOL __stdcall DllMain(HINSTANCE hModule, DWORD ul_reason_for_call, LPVOID lpReserved)
{
	...
	case DLL_PROCESS_ATTACH:
		...
		CreateThread(NULL, NULL, CheatMainThread, hModule, NULL, NULL);
		break;
	}
	return TRUE;
}
